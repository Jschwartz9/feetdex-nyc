...
print("🔧 Implementing complete shoe game functionality...")

with open('feetdex-nyc.html', 'r') as f:
    content = f.read()

# 1. Add the educational questions and question UI if not already present
if 'SHOE_QUESTIONS' not in content:
    print("📚 Adding educational shoe questions...")
...
'''
    ...
    # Insert questions after SHOE_CATALOG
    shoe_catalog_pos = content.find('class ShoeDex {')
    if shoe_catalog_pos > 0:
        content = content[:shoe_catalog_pos] + questions_code + '\n' + content[shoe_catalog_pos:]

# 2. Add Question Manager class if not present
if 'class QuestionManager' not in content:
    print("🎯 Adding question management system...")

    question_manager_code = '''
      ...
      const questionHTML = `
        <div id="question-overlay" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.95); z-index: 300;
          ...
          <div id="question-container" style="max-width: 600px; padding: 40px; text-align: center; border: 2px solid #fff; background: rgba(0,0,0,0.8);">
            ...
            <div id="question-options" style="display: grid; gap: 15px;"></div>
...
'''
    ...
    # Insert before main game initialization
    main_js_pos = content.find('// ── main.js')
    if main_js_pos > 0:
        content = content[:main_js_pos] + question_manager_code + '\n' + content[main_js_pos:]

# 3. Change all feet references to shoes in dialogue and text
print("👟 Converting all feet references to shoes...")
...
feet_to_shoes = [
    ...
    ('Foot', 'Shoe'),
    ('FOOT', 'SHOE'),
    # Keep some specific game terms
    ('feetDex', 'shoeDex'),
    ('FeetDex', 'ShoeDex'),
    ('FEETDEX', 'SHOEDEX'),
    # Dialogue specific
    ('pair of feet', 'pair of shoes'),
    ('these feet', 'these shoes'),
    ('your feet', 'your shoes'),
    ('my feet', 'my shoes'),
    ('his feet', 'his shoes'),
    ('her feet', 'her shoes'),
    ('their feet', 'their shoes'),
    # Collection related
    ('collect feet', 'collect shoes'),
    ('found feet', 'found shoes'),
    ('rare feet', 'rare shoes'),
    ('legendary feet', 'legendary shoes'),
    ('unique feet', 'unique shoes'),
]

for old, new in feet_to_shoes:
...
print("✅ Added educational questions system")
print("✅ Changed all feet references to shoes")
print("✅ Integrated 15-second timer functionality")
print("🎮 Game is now ready for shoe collection with educational questions!")